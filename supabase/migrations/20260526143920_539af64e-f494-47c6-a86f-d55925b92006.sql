  ...
  WITH filtered AS MATERIALIZED (
    ...
    SELECT
      *,
      ...
      (likes_count + replies_count + shares_count) AS engagement
    FROM filtered
  ), agg AS (
    SELECT
    ...
    SELECT
      ...
      coalesce(sum(replies_count), 0)::bigint AS comentarios_respostas,
    ...
    JOIN theme_patterns p ON EXISTS (
      ...
      WHERE lower(coalesce(e.comment_text, '')) LIKE '%' || kw || '%'
    )
    GROUP BY p.theme
