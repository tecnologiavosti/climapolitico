ALTER TABLE public.social_interactions
  ...
  ADD COLUMN IF NOT EXISTS engagement_score bigint;
...
AS $function$
  SELECT CASE
    WHEN public.norm_text(coalesce(_network, '')) IN ('youtube','yt') THEN 'youtube'
    ...
    WHEN public.norm_text(coalesce(_network, '')) IN ('twitter','twitter/x','x','twitter_x') THEN 'twitter'
    WHEN public.norm_text(coalesce(_network, '')) IN ('google news','google_news','googlenews') THEN 'google_news'
    WHEN public.norm_text(coalesce(_network, '')) IN ('gdelt','portal','portais','noticias','news','portal de noticia','portal de noticias') THEN 'portal'
...
BEGIN
  ...
  NEW.engagement_score := (coalesce(NEW.likes_count, 0) + coalesce(NEW.replies_count, 0) + coalesce(NEW.shares_count, 0))::bigint;
...
DROP TRIGGER IF EXISTS trg_social_interactions_enrich_post_metadata ON public.social_interactions;
CREATE TRIGGER trg_social_interactions_enrich_post_metadata
  BEFORE INSERT OR UPDATE OF social_network, platform, likes_count, replies_count, shares_count, post_url, author_profile_url, post_title, post_description, author_name, comment_author, comment_text, interaction_type
  ON public.social_interactions
  FOR EACH ROW EXECUTE FUNCTION public.social_interactions_enrich_post_metadata();

CREATE INDEX IF NOT EXISTS idx_social_interactions_platform_rank
  ON public.social_interactions (user_id, candidate_id, platform, engagement_score DESC, collected_at DESC)
  WHERE is_political_content = true AND invalidated_at IS NULL;
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      coalesce(si.engagement_score, (coalesce(si.likes_count,0) + coalesce(si.replies_count,0) + coalesce(si.shares_count,0))::bigint) AS engagement,
      CASE
    ...
    SELECT *,
      ...
      count(*) OVER (PARTITION BY canonical_platform) AS platform_total
    FROM eligible
  ), diversified AS (
    SELECT *
