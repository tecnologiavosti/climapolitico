CREATE OR REPLACE FUNCTION public.nv_is_portuguese_text(p_text text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
SET search_path TO 'public'
AS $$
  SELECT COALESCE(NULLIF(btrim(p_text), ''), '') <> ''
    AND (
      lower(p_text) ~ '(á|à|â|ã|é|ê|í|ó|ô|õ|ú|ç)'
      OR lower(p_text) ~ '(^|[^[:alpha:]])(a|o|os|as|um|uma|uns|umas|de|do|da|dos|das|no|na|nos|nas|que|não|para|por|com|sem|sobre|entre|mais|muito|foi|são|ser|ter|vai|como|quando|onde|porque|brasil|brasileiro|brasileira|política|politico|político|eleição|eleicoes|eleições|candidato|candidata|governo|presidente|deputado|senador|prefeito|ministro|congresso|stf|tse)([^[:alpha:]]|$)'
    );
$$;
...
BEGIN
  ...
  WITH current_rows AS (
    ...
    SELECT
      ...
      coalesce(sum(comments),0)::bigint AS replies,
      ...
      (SELECT count(*) FILTER (WHERE sent = 'positive') FROM previous_rows)::bigint AS prev_pos,
      (SELECT count(*) FILTER (WHERE sent = 'negative') FROM previous_rows)::bigint AS prev_neg,
      (SELECT count(*) FILTER (WHERE sent NOT IN ('positive','negative')) FROM previous_rows)::bigint AS prev_neu
    FROM current_rows
  ), series AS (
    ...
    SELECT network,
      ...
      coalesce(sum(shares),0)::bigint AS shares,
      coalesce(sum(likes + comments + shares),0)::bigint AS engagement,
      (count(*)::numeric * 0.4 + coalesce(sum(likes + comments + shares),0)::numeric * 0.6) AS dominance
    FROM current_rows
    GROUP BY 1
  ), heat AS (
  ...
  WITH cur_matches AS (
    SELECT lower((m)[1]) AS tag,
      COALESCE(public.network_view_sentiment(si.sentiment_label, si.sentiment_score, concat_ws(' ', si.comment_text, si.post_title, si.post_description)), 'neutral') AS sent
    FROM public.social_interactions si
    CROSS JOIN LATERAL regexp_matches(concat_ws(' ', si.post_title, si.post_description, si.comment_text), '#([[:alnum:]_]+)', 'g') AS m
    WHERE si.invalidated_at IS NULL
      AND (v_is_admin OR si.user_id = v_uid)
      AND (p_candidate_id IS NULL OR si.candidate_id = p_candidate_id)
      AND (v_network IS NULL OR public.nv_network_key(si.social_network) = v_network)
  ...
  WITH base AS (
    ...
    WHERE si.invalidated_at IS NULL
      ...
      AND (v_is_total_period OR COALESCE(si.original_posted_at, si.collected_at, si.created_at) >= v_since)
      AND COALESCE(si.original_posted_at, si.collected_at, si.created_at) <= now()
      AND COALESCE(NULLIF(si.comment_text,''), NULLIF(si.post_title,''), NULLIF(si.post_description,'')) IS NOT NULL
      AND public.nv_is_portuguese_text(concat_ws(' ', si.post_title, si.post_description, si.comment_text))
      AND (COALESCE(si.likes_count,0) + COALESCE(si.replies_count,0) + COALESCE(si.shares_count,0) + COALESCE(si.engagement_score,0)) > 0
  ), deduped AS (
  ...
  RETURN jsonb_build_object(
    ...
    'diagnostics', jsonb_build_object(
      'cache_hit', false,
      'source', 'social_interactions_raw',
...
REVOKE ALL ON FUNCTION public.nv_is_portuguese_text(text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.nv_is_portuguese_text(text) TO authenticated, service_role;
