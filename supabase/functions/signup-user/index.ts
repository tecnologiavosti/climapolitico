// signup-user: cria conta sem disparar e-mail nativo do backend.
// Usa admin.createUser com email_confirm=true — nenhum template padrão de auth é enviado.
    ...
    try {
      const html = `
        <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;color:#0f172a;">
          ...
          <p style="margin:0 0 12px;font-size:15px;">Sua conta foi criada com sucesso. Bem-vindo(a) ao ${EMAIL_BRAND}.</p>
      ...
      await sendAuthEmail({
        to: email,
        subject: `Bem-vindo(a) ao ${EMAIL_BRAND}`,
        html,
        text: `Bem-vindo(a) ao ${EMAIL_BRAND}! Acesse: https://climapolitico.com.br/auth`,
      });
    } catch (e) {
