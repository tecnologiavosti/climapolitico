          ...
          for (const chunk of chunkArray(pageCandidates, BATCH_SIZE)) {
            ...
            const batchToInsert = chunk.map((c, idx) => {
              ...
              return {
                ...
                post_title: video.snippet?.title || null,
                post_description: video.snippet?.description || null,
                thumbnail_url: video.snippet?.thumbnails?.high?.url || video.snippet?.thumbnails?.medium?.url || video.snippet?.thumbnails?.default?.url || `https://img.youtube.com/vi/${videoId}/hqdefault.jpg`,
                author_name: video.snippet?.channelTitle || c.author,
